...
const GuideFilter = ({ categoryname, match, catId }) => {
  ...
  async function fetchItem() {
    ...
    try {
      const response = await axios.get(
        `http://178.128.21.17/api/guides?type=${id}&lastId=&lang=${stringLanguage}&catId=${catId}`
      );
  ...
  const fetchMoreItems = () =>{
    ...
    async function fetchMoreItems() {
      ...
      try {
        const response = await axios.get(
          `http://178.128.21.17/api/guides?type=${id}&lastId=${islast}&lang=${stringLanguage}&catId=${catId}`
        );

...
const GetBlogList = () => {
  ...
  async function fetchItems() {
    try {
      const response = await axios.get(`http://178.128.21.17/api/blogs/?lang=${blogLanguage}&lastId=`);
  ...
  async function fetchMoreItems() {
    ...
    try {
      const response = await axios.get(`http://178.128.21.17/api/blogs/?lang=${blogLanguage}&lastId=${islast}`);
